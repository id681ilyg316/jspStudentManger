		...
		try{
			...
			if(type.equals("tea")){//判断是否为老师
				...
				if(tea!=null){
					...
					response.sendRedirect("/jspStudentManager/tea/teacher.jsp");
				}else{
					out.println("T用户名或密码不正确，请核对后重试!!");
					response.setHeader("refresh","2;url=/jspStudentManager/tea/teaLogin.jsp");
				}
			}else{
				out.println("错误！请登录！");
				response.setHeader("refresh","2;url=/jspStudentManager/Login.jsp");
			}
		}catch(Exception e){ e.printStackTrace();response.sendRedirect("/jspStudentManager/Login.jsp");}
		out.flush();
		out.close();
		}
